...
class NotefulApp extends React.Component {
  ...
  getNoteData() {
    fetch(`https://polar-sierra-90172.herokuapp.com/api/note`)
      .then((response) => {
        if (!response.ok) {
          throw new Error('Error');
        }
      ...
      <div className='NotefulApp'>
        <NotefulContext.Provider value={contextValue}>
          ...
          <div className='flex-three'>
            <Switch>
              <Route exact path='/' component={NoteStorage} />
              <Route path='/folder/:folderId' component={NoteStorageFolder} />
              <Route path='/note/:noteId' component={NoteStorageNote} />
